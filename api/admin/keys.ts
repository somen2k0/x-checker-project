...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET /api/admin/keys — list configured keys (masked)
  if (req.method === "GET") {
    ...
    res.json({
      ...
      hardcodedKeyCount: HARDCODED_KEYS.length,
      envVarSet: envKeys.length > 0,
    });
    return;
  }

  // POST /api/admin/keys — test a new key provided by the user
  if (req.method === "POST") {
    const body = req.body as { key?: string; source?: string; index?: number; action?: string };

    // Test a server-side configured key by source + index
    if (body.action === "test-server") {
      const pool = body.source === "env"
        ? (process.env.RAPIDAPI_KEYS ?? "").split(",").map((k) => k.trim()).filter((k) => k.length > 0)
        : HARDCODED_KEYS;
    ...
    // Test a new key provided directly
