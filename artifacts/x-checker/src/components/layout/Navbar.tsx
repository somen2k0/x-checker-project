...
const TEMP_MAIL_ITEMS = [
  { icon: Inbox,  label: "Disposable Inbox", href: "/tools/temp-mail/disposable",  desc: "Anonymous throwaway inbox" },
  { icon: Mail,   label: "Temp Gmail",        href: "/tools/temp-mail/tempgmail",   desc: "Real Gmail via Gmailnator" },
...
function TempMailNavItem({ currentPath }: { currentPath: string }) {
  ...
  const isActive = currentPath.startsWith("/tools/temp-mail") || currentPath === "/tools/temp-gmail" || open;
  ...
  return (
    <div className="relative" onMouseEnter={enter} onMouseLeave={leave}>
      ...
      {open && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-56 z-50 rounded-xl border border-border/60 bg-background/98 backdrop-blur-xl shadow-2xl shadow-black/30 animate-in fade-in slide-in-from-top-1 duration-150">
          <div className="p-2 space-y-0.5">
            {TEMP_MAIL_ITEMS.map(({ icon: Icon, label, href, desc }) => (
              <Link key={href} href={href} onClick={close}>
                <div className={`group flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-muted/60 transition-colors cursor-pointer ${currentPath === href ? "bg-muted/40" : ""}`}>
